...
async function testPagination() {
  ...
  try {
    ...
    console.log('---------------------');
    const result1 = await agent.process('show all my reminders', conversationState);
    console.log(JSON.stringify(result1, null, 2));
    
    // Test pagination request
    console.log('\n\nTEST 2: Pagination Request');
    console.log('-------------------------');
    const result2 = await agent.process('show more', conversationState);
    console.log(JSON.stringify(result2, null, 2));
    
    // Test another pagination request
    console.log('\n\nTEST 3: Another Pagination Request');
    console.log('-------------------------------');
    const result3 = await agent.process('next page', conversationState);
    console.log(JSON.stringify(result3, null, 2));
    
    // Test a query with specific filter
    console.log('\n\nTEST 4: Query with Specific Filter');
    console.log('--------------------------------');
    const result4 = await agent.process('show my important reminders', conversationState);
    ...
    console.log('-----------------------------------------------');
    const result5 = await agent.process('show my next 11 reminders', conversationState);
